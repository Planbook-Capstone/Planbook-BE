...
public interface LessonPlanNodeRepository extends JpaRepository<LessonPlanNode, Long> {
     ...
     */
    List<LessonPlanNode> findByLessonPlanIdAndParentIsNullAndStatusOrderByOrderIndex(Long lessonPlanId, Status status);
     ...
     */
    List<LessonPlanNode> findByLessonPlanIdAndParentIsNullOrderByOrderIndex(Long lessonPlanId);
     ...
     */
    @Query("SELECT COALESCE(MAX(n.orderIndex), 0) FROM LessonPlanNode n WHERE n.lessonPlanId = :lessonPlanId AND n.parent IS NULL")
    Integer findMaxOrderIndexByLessonPlanIdAndParentIsNull(@Param("lessonPlanId") Long lessonPlanId);
