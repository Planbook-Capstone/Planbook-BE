...
public interface PaymentTransactionRepository extends JpaRepository<PaymentTransaction, UUID> {
    ...
    boolean existsByOrderIdAndStatus(UUID orderId, PaymentStatusEnum status);

    Optional<PaymentTransaction> findByParentTransactionId(UUID parentTransactionId);

    @Query("SELECT p FROM PaymentTransaction p WHERE p.orderId = :orderId AND p.parentTransactionId IS NULL")
    ...
    boolean existsByOrderIdAndGatewayAndStatus(UUID orderId, GatewayEnum gateway, PaymentStatusEnum status);
